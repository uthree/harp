...
impl Lowerer {
    ...
    fn lower_node(&mut self, node: &GraphNode) -> LoweringResult<Option<AstNode>> {
        ...
        let result = match node.op() {
            ...
            GraphOp::MatMul {
                transpose,
                accumulator_dtype,
            } => self.lower_matmul(node, &output_buf, *transpose, accumulator_dtype.clone()),
        };
    ...
    /// Lower a MatMul operation
    ///
    /// MatMul nodes are generated by graph-level optimization and represent
    /// matrix multiplication: C[M, N] = A[M, K] @ B[K, N].
    ///
    /// For F16 with aligned dimensions, generates WmmaMatmul AST node.
    /// Otherwise, generates an efficient 3-loop implementation.
    #[allow(clippy::too_many_arguments)]
    fn lower_matmul(
        &mut self,
        node: &GraphNode,
        ...
        // Get buffer names
        let a_buf = self.get_buffer_name(a);
        let b_buf = self.get_buffer_name(b);

        // Extract dimensions
        let a_shape = a.shape();
        let b_shape = b.shape();
        let _output_shape = node.shape();

        // Determine M, K, N based on transpose flags
        let (m, k_a) = if trans_a {
            (a_shape.get(1), a_shape.first())
        } else {
            (a_shape.first(), a_shape.get(1))
        };

        let (_k_b, n) = if trans_b {
            (b_shape.get(1), b_shape.first())
        } else {
            (b_shape.first(), b_shape.get(1))
        };

        // Fall back to default values if shapes are incomplete
        let m = m.cloned().unwrap_or(Expr::Const(1));
        let k = k_a.cloned().unwrap_or(Expr::Const(1));
        let n = n.cloned().unwrap_or(Expr::Const(1));

        // Convert Expr to AstNode
        let m_ast: AstNode = m.clone().into();
        let k_ast: AstNode = k.clone().into();
        let n_ast: AstNode = n.clone().into();

        let dtype = node.dtype().clone();
        let acc_dtype = accumulator_dtype.unwrap_or(DType::F32);

        // Check if WMMA is eligible
        let wmma_eligible = self.is_wmma_eligible(&dtype, &m, &k, &n);

        if wmma_eligible {
            // Generate WmmaMatmul node
            let a_stride = if trans_a { m.clone() } else { k.clone() };
            let b_stride = if trans_b { k.clone() } else { n.clone() };
            let c_stride = n.clone();

            let body = AstNode::WmmaMatmul {
                a_ptr: Box::new(AstNode::Var(a_buf)),
                a_offset: Box::new(AstNode::Const(Literal::I64(0))),
                a_stride: Box::new(a_stride.into()),
                b_ptr: Box::new(AstNode::Var(b_buf)),
                b_offset: Box::new(AstNode::Const(Literal::I64(0))),
                b_stride: Box::new(b_stride.into()),
                c_ptr: Box::new(AstNode::Var(output_buf.to_string())),
                c_offset: Box::new(AstNode::Const(Literal::I64(0))),
                c_stride: Box::new(c_stride.into()),
                m: Box::new(m_ast),
                k: Box::new(k_ast),
                n: Box::new(n_ast),
                dtype_ab: dtype,
            ...
            // Generate efficient 3-loop matmul
            // for i in 0..M:
            //     for j in 0..N:
            //         acc = 0
            //         for k in 0..K:
            //             acc += A[i, k] * B[k, j]  (with transpose adjustments)
            //         C[i, j] = acc
            ...
            // Compute A and B indices based on transpose flags
            let a_idx = if trans_a {
                // A[k, i] when transposed
                AstNode::Add(
                    Box::new(AstNode::Mul(
                        Box::new(AstNode::Var(k_var.to_string())),
                        Box::new(m.clone().into()),
                    )),
                    Box::new(AstNode::Var(i_var.to_string())),
                )
            } else {
                // A[i, k] when not transposed
                AstNode::Add(
                    Box::new(AstNode::Mul(
                        Box::new(AstNode::Var(i_var.to_string())),
                        Box::new(k.clone().into()),
                    )),
                    Box::new(AstNode::Var(k_var.to_string())),
                )
            };
            ...
            let b_idx = if trans_b {
                // B[j, k] when transposed
                AstNode::Add(
                    Box::new(AstNode::Mul(
                        Box::new(AstNode::Var(j_var.to_string())),
                        Box::new(k.clone().into()),
                    )),
                    Box::new(AstNode::Var(k_var.to_string())),
                )
            } else {
                // B[k, j] when not transposed
                AstNode::Add(
                    Box::new(AstNode::Mul(
                        Box::new(AstNode::Var(k_var.to_string())),
                        Box::new(n.clone().into()),
                    )),
                    Box::new(AstNode::Var(j_var.to_string())),
                )
            };
            ...
            let a_load = AstNode::Load {
                ptr: Box::new(AstNode::Var(a_buf)),
                offset: Box::new(a_idx),
                count: 1,
                dtype: a.dtype().clone(),
            };

            let b_load = AstNode::Load {
                ptr: Box::new(AstNode::Var(b_buf)),
                offset: Box::new(b_idx),
                count: 1,
                dtype: b.dtype().clone(),
            };

            // acc = acc + a * b
            let mul = AstNode::Mul(Box::new(a_load), Box::new(b_load));
            let add = AstNode::Add(Box::new(AstNode::Var("acc".to_string())), Box::new(mul));

            let k_body = AstNode::Assign {
                var: "acc".to_string(),
                value: Box::new(add),
            };
            ...
            let store = AstNode::Store {
                ptr: Box::new(AstNode::Var(output_buf.to_string())),
                offset: Box::new(c_idx),
                value: Box::new(AstNode::Var("acc".to_string())),
            };

            // Block with acc declaration
            let mut scope = Scope::new();
            scope
                .declare("acc".to_string(), acc_dtype.clone(), Mutability::Mutable)
                .unwrap();

            let acc_init = match acc_dtype {
                DType::F16 => AstNode::Const(Literal::F16(half::f16::from_f32(0.0))),
                DType::F32 => AstNode::Const(Literal::F32(0.0)),
                DType::F64 => AstNode::Const(Literal::F64(0.0)),
            ...
            self.finalize_kernel(ctx, m_loop)
        }
    }

    /// Check if WMMA is eligible for this matmul
    fn is_wmma_eligible(&self, dtype: &DType, m: &Expr, k: &Expr, n: &Expr) -> bool {
        // Must be F16
        ...
        let m_ok = m.as_const().is_some_and(|v| v > 0 && v % 16 == 0);
        let k_ok = k.as_const().is_some_and(|v| v > 0 && v % 16 == 0);
        let n_ok = n.as_const().is_some_and(|v| v > 0 && v % 16 == 0);
