...
impl Lowerer {
    ...
    fn compute_total_size(&self, view: &crate::graph::shape::view::View) -> Option<usize> {
        let crate::graph::shape::view::View::Linear { shape, .. } = view;
        let mut total = 1;
        for dim in shape {
    ...
    fn create_fused_elementwise_loop(
        ...
        if dim >= shape.len() {
            ...
            // 結果を格納
            let result_index = self.compute_memory_index(strides, offset, shape.len());
            return AstNode::Store {
                target: Box::new(AstNode::Var(result_var.to_string())),
                index: Box::new(result_index),
    ...
    fn replace_captures_with_input_refs(
        &self,
        ast: &AstNode,
        input_vars: &[String],
        inputs: &[GraphNode],
        ...
        match ast {
            AstNode::Capture(idx) => {
                ...
                // 入力のindexを計算
                let input_index = self.compute_memory_index(input_strides, input_offset, ndim);
