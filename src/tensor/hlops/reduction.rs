...
impl<D: Dimension> Tensor<f32, D> {
    ...
    pub fn sum(&self, axes: &[usize], keepdim: bool) -> Tensor<f32, DimDyn> {
    ...
    pub fn mean(&self, axes: &[usize], keepdim: bool) -> Tensor<f32, DimDyn> {
    ...
    ///
    /// Var(x) = Mean((x - mean)^2)
    pub fn var(&self, axes: &[usize], keepdim: bool) -> Tensor<f32, DimDyn> {
        // Compute mean
    ...
    ///
    /// Std(x) = Sqrt(Var(x))
    pub fn std(&self, axes: &[usize], keepdim: bool) -> Tensor<f32, DimDyn> {
    ...
    ///
    /// Softmax(x) = Exp(x - max) / Sum(Exp(x - max))
    pub fn softmax(&self, axis: usize) -> Tensor<f32, DimDyn> {
    ...
    ///
    /// LogSoftmax(x) = x - max - Log(Sum(Exp(x - max)))
    pub fn log_softmax(&self, axis: usize) -> Tensor<f32, DimDyn> {
    ...
    pub fn layer_norm(&self, normalized_axes: &[usize], eps: f32) -> Tensor<f32, DimDyn> {
