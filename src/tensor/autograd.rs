...
/// Gradient function for ReLU activation: max(x, 0)
#[derive(Debug)]
pub struct ReLUBackward;

impl GradFn for ReLUBackward {
    fn backward(&self, grad_output: GraphNode, inputs: &[GraphNode]) -> Vec<Option<GraphNode>> {
        // For z = ReLU(x) = max(x, 0):
        // dL/dx = dL/dz * dz/dx
        // dz/dx = 1 if x > 0, else 0
        //
        // mask = (x > 0)  つまり less_than(0, x)
        // grad_x = grad_output * mask.cast(grad_output.dtype)
        assert_eq!(inputs.len(), 1, "ReLUBackward expects 1 input");
        let x = &inputs[0];

        let zero = GraphNode::f32(0.0);
        // x > 0 は less_than(0, x) として実装
        let mask = zero.less_than(x.clone());
        // Bool型をF32にキャスト
        let mask_float = mask.cast(grad_output.dtype.clone());
        let grad_x = grad_output * mask_float;
...
/// Gradient function for Sigmoid activation: 1 / (1 + exp(-x))
#[derive(Debug)]
pub struct SigmoidBackward;

impl GradFn for SigmoidBackward {
    fn backward(&self, grad_output: GraphNode, inputs: &[GraphNode]) -> Vec<Option<GraphNode>> {
        // For z = sigmoid(x) = 1 / (1 + exp(-x)):
        // dL/dx = dL/dz * dz/dx
        // dz/dx = sigmoid(x) * (1 - sigmoid(x))
        //
        // 効率のため、forward passの結果を再計算せずに使う場合もあるが、
        // ここでは再計算する
        assert_eq!(inputs.len(), 1, "SigmoidBackward expects 1 input");
        let x = &inputs[0];

        // sigmoid(x) = 1 / (1 + exp(-x))
        let one = GraphNode::f32(1.0);
        let sigmoid_x = (one.clone() + (-x.clone()).exp()).recip();

        // sigmoid(x) * (1 - sigmoid(x))
        let grad_x = grad_output * sigmoid_x.clone() * (one - sigmoid_x);
...
/// Gradient function for Tanh activation: tanh(x)
#[derive(Debug)]
...
impl GradFn for TanhBackward {
    fn backward(&self, grad_output: GraphNode, inputs: &[GraphNode]) -> Vec<Option<GraphNode>> {
        ...
        assert_eq!(inputs.len(), 1, "TanhBackward expects 1 input");
        let x = &inputs[0];

        // tanh(x) = (exp(x) - exp(-x)) / (exp(x) + exp(-x))
        let exp_x = x.clone().exp();
        let exp_neg_x = (-x.clone()).exp();
        let tanh_x = (exp_x.clone() - exp_neg_x.clone()) / (exp_x + exp_neg_x);

        // 1 - tanh(x)^2
        let one = GraphNode::f32(1.0);
        let grad_x = grad_output * (one - tanh_x.clone() * tanh_x);
