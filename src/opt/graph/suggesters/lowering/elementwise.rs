...
pub fn build_elementwise_function_simd(
    ...
    // スカラー（ndim=0）または1次元で最内軸がSIMD幅より小さい場合はスキップ
    if ndim == 0 {
        return None;
    }
    if let Some(Expr::Const(size)) = shape.last()
    ...
    Some(build_elementwise_function_simd_impl(
        ndim,
        num_inputs,
        expr_with_consts,
        &node.dtype,
...
pub fn build_fused_elementwise_function_simd(
    ...
    // スカラー（ndim=0）または最内軸がSIMD幅より小さい場合はスキップ
    if ndim == 0 {
        return None;
    }
    if let Some(Expr::Const(size)) = shape.last()
...
/// SIMD版Elementwise関数の実装
fn build_elementwise_function_simd_impl(
    ndim: usize,
    num_inputs: usize,
    expr: AstNode,
    output_dtype: &GraphDType,
    ...
    let simd_body = {
        ...
        let final_expr = expr.substitute(&mappings);
        let store_stmt = store(var(ph::OUTPUT), offset.clone(), final_expr);
        vec![store_stmt]
    };
    ...
    let scalar_body = {
        let mut mappings = HashMap::new();
        for i in 0..num_inputs {
            let load_node = load(var(ph::input(i)), offset.clone(), scalar_dtype.clone());
            mappings.insert(i.to_string(), load_node);
        }
        let final_expr = expr.substitute(&mappings);
        let store_stmt = store(var(ph::OUTPUT), offset.clone(), final_expr);
    ...
    let body = wrap_with_simd_innermost_loop(ndim, simd_body, scalar_body, shape, simd_width);
