...
impl FusionSuggester {
    ...
    fn detect_elementwise_chain(
        ...
        // このノードのFusedElementwiseOpを作成
        let inputs: Vec<FusedInput> = node
            .src
            .iter()
            .map(|src| {
                let idx = input_mapping[&src.as_ptr()];
        ...
        // 2つ以上のElementwise演算がチェーンになっている場合のみ融合
        // (現状では1つのノードのみなので、チェーン検出は将来の拡張)
        if ops.len() < 2 {
            return None;
        }
