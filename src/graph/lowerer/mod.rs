...
mod tests {
    ...
    #[test]
    fn test_lower_fused_reduce_multi_axis() {
        setup_logger();
        use crate::graph::shape::expr::Expr;

        let graph = Graph::new();
        let a = graph.input(DType::F32, vec![10.into(), 20.into(), 30.into()]); // buffer 0
        ...
        if let AstOp::Func { .. } = &kernel_impl.op {
            ...
            if let AstOp::Block { .. } = &fused_reduce_block.op {
                ...
                if let AstOp::Range { .. } = &fused_reduce_ast.op {
                    ...
                    // block contains [init_acc, inner_loop, store_result]
                    assert!(matches!(block.src[0].op, AstOp::Declare { .. })); // init_acc
                    assert!(matches!(block.src[1].op, AstOp::Range { .. })); // inner_loop_1
                    assert!(matches!(block.src[2].op, AstOp::Store)); // store_result

                    // Check the nested inner loops
                    let inner_loop_1 = &block.src[1];
                    let inner_block_1 = &inner_loop_1.src[1];
                    assert!(matches!(inner_block_1.op, AstOp::Range { .. })); // inner_loop_2

                    let inner_loop_2 = inner_block_1;
                    let inner_block_2 = &inner_loop_2.src[1];
                    assert!(matches!(inner_block_2.op, AstOp::Assign)); // acc update
                } else {
                    panic!("Expected outer loop, found {:?}", fused_reduce_ast.op);
                }
            } else {
