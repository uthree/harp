...
impl GradFn for Conv1dBackward {
    fn apply(&self, grad_output: &Tensor, inputs: &[Tensor]) -> Vec<Option<Tensor>> {
        ...
        let _l_in = input_shape[1].expect_usize("Conv1d backward requires constant input shape");
        let kernel_size =
        ...
        let c_out = kernel_shape[0].expect_const("Conv1d backward requires constant kernel shape");
...
impl GradFn for Conv2dBackward {
    fn apply(&self, grad_output: &Tensor, inputs: &[Tensor]) -> Vec<Option<Tensor>> {
        ...
        let h_out_val =
            reduced_shape[3].expect_usize("Conv2d backward requires constant grad_output shape");
        let w_out_val =
            reduced_shape[4].expect_usize("Conv2d backward requires constant grad_output shape");
        ...
        let c_out = kernel_shape[0].expect_const("Conv2d backward requires constant kernel shape");
