...
use harp_ir::dtype::DType;
use harp_ir::node::{self, Node, constant};
use harp_ir::op::{
    ...
    pub fn lower(&self, shape_tracker: &ShapeTracker) -> Node {
        ...
        let compiled_node = match op.name() {
            ...
            "Load" => {
                ...
                Node::new(Box::new(Load), vec![buffer_node, index_node])
            }
            ...
            "Reduce" => {
                ...
                for i in 0..dim_size {
                    ...
                    accumulator = Node::new(reduce_op.op.clone(), vec![accumulator, term]);
                }
                accumulator
            }
            "OpAdd" | "OpSub" | "OpMul" | "OpDiv" => {
                ...
                Node::new(op.clone(), vec![left, right])
            }
