...
fn test_eval_unary_functions(#[case] graph: harp_ir::node::Node, #[case] expected: f32) {
...
fn test_eval_binary_functions(#[case] graph: harp_ir::node::Node, #[case] expected: f32) {
    let rewriter = default_rewriter();
    let rewritten_graph = rewriter.rewrite(graph);
    let result = rewritten_graph
        .op()
