    ...
    T: DType + PartialEq + std::fmt::Debug,
{
    ...
    if let Some(const_op) = source_tensor.data.op.as_any().downcast_ref::<Const>() {
        assert_eq!(*const_op.0.as_any().downcast_ref::<T>().unwrap(), value);
    } else {
...
fn test_tensor_zeros() {
    ...
    check_full_structure(&tensor, &shape, 0.0f64);
}
...
fn test_tensor_ones() {
    ...
    check_full_structure(&tensor, &shape, 1.0f64);
}
...
#[test]
fn test_tensor_full_with_integer() {
    let shape = vec![2, 2];
    let tensor = Tensor::full(shape.clone(), 42i32);
    check_full_structure(&tensor, &shape, 42i32);
}
