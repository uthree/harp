...
impl Kernel for CudaKernel {
    ...
    fn execute_with_sizes(
        ...
        let grid_dim = (
            (global_size[0] / local_size[0].max(1)) as u32,
            (global_size[1] / local_size[1].max(1)) as u32,
            (global_size[2] / local_size[2].max(1)) as u32,
        );
        let block_dim = (
            local_size[0] as u32,
            local_size[1] as u32,
        ...
        let mut _kernel_args: Vec<u64> = Vec::with_capacity(inputs.len() + outputs.len());
        ...
        for input in inputs {
            ...
            _kernel_args.push(cuda_buf.as_device_ptr_u64());
        }
        ...
        // TODO: Implement proper kernel launch using cudarc's API
        Err(Box::new(CudaKernelError::NotImplemented(
            "Kernel launch not fully implemented yet".to_string()
        )))
    }
