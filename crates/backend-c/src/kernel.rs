//! C kernel implementation for C backend
//!
...
unsafe impl Send for CKernel {}
unsafe impl Sync for CKernel {}

impl CKernel {
    /// Create a new CKernel from a compiled library
    pub fn new(
        temp_dir: TempDir,
        lib_path: PathBuf,
        config: KernelConfig,
    ...
    unsafe fn execute_with_ptrs(
        ...
        // The kernel function signature depends on the number of arguments
        // SAFETY: We verified the entry point exists in the constructor
        match args.len() {
            1 => {
...
impl Kernel for CKernel {
        ...
        let input_ptrs: Vec<*const u8> = inputs
            ...
            .map(|b| {
                b.as_any()
                    ...
                    .expect("Input buffer must be CBuffer")
                    .as_ptr()
            })
            .collect();
        ...
        let output_ptrs: Vec<*mut u8> = outputs
            ...
            .map(|b| {
                b.as_any_mut()
                    ...
                    .expect("Output buffer must be CBuffer")
                    .as_mut_ptr()
            })
            .collect();
    ...
    fn execute_with_sizes(
        ...
        // C backend ignores work sizes (sequential execution)
...
impl std::fmt::Debug for CKernel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CKernel")
