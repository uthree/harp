//! CPU buffer implementation for C backend
//!
...
/// Error type for CBuffer operations
#[derive(Debug, Clone)]
pub enum CBufferError {
    /// Size mismatch error
...
impl fmt::Display for CBufferError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CBufferError::SizeMismatch { expected, actual } => {
                write!(
                    f,
...
impl Error for CBufferError {}

/// CPU buffer for C backend
///
...
impl Buffer for CBuffer {
    ...
    fn write_from_host(&mut self, data: &[u8]) -> Result<(), Box<dyn Error + Send + Sync>> {
        if data.len() != self.data.len() {
            return Err(Box::new(CBufferError::SizeMismatch {
...
mod tests {
    ...
    fn test_buffer_allocate() {
        ...
        let buffer = CBuffer::allocate(&device, vec![2, 3], DType::F32).unwrap();
    ...
    fn test_buffer_write_read() {
        let device = CDevice::new();
        let mut buffer = CBuffer::allocate(&device, vec![4], DType::F32).unwrap();
    ...
    fn test_buffer_clone() {
        let device = CDevice::new();
        let mut buffer = CBuffer::allocate(&device, vec![2], DType::F32).unwrap();
