...
impl Rewriter {
    ...
    fn apply_to_children(&self, uop: &Rc<UOp>) -> (Rc<UOp>, bool) {
        match &**uop {
            ...
            | UOp::Var { .. }
            | UOp::Barrier
            | UOp::Wildcard(_) => (uop.clone(), false),
            ...
            UOp::Add(lhs, rhs) => {
                ...
                if lhs_changed || rhs_changed {
                    (Rc::new(UOp::Add(new_lhs, new_rhs)), true)
                } else {
                    (uop.clone(), false)
                }
            }
            ...
            UOp::Mul(lhs, rhs) => {
                ...
                let (new_rhs, rhs_changed) = self
                    ...
                    .unwrap_or((rhs.clone(), false));
                if lhs_changed || rhs_changed {
                    (Rc::new(UOp::Mul(new_lhs, new_rhs)), true)
                } else {
                    (uop.clone(), false)
                }
            }
            ...
            UOp::Max(lhs, rhs) => {
                ...
                let (new_rhs, rhs_changed) = self
                    ...
                    .unwrap_or((rhs.clone(), false));
                if lhs_changed || rhs_changed {
                    (Rc::new(UOp::Max(new_lhs, new_rhs)), true)
                } else {
                    (uop.clone(), false)
                }
            }
            ...
            UOp::Sequence(ops) => {
                ...
                let new_ops: Vec<Rc<UOp>> = ops
                    ...
                    .collect();
                if changed {
                    (Rc::new(UOp::Sequence(new_ops)), true)
                } else {
                    (uop.clone(), false)
                }
            }
...
fn pattern_match_impl(
    ...
    match (&**pattern, &**uop) {
        ...
        // 二項演算
        (UOp::Add(l1, r1), UOp::Add(l2, r2))
        | (UOp::Mul(l1, r1), UOp::Mul(l2, r2))
        | (UOp::Max(l1, r1), UOp::Max(l2, r2))
        | (UOp::Rem(l1, r1), UOp::Rem(l2, r2))
        | (UOp::Idiv(l1, r1), UOp::Idiv(l2, r2))
        | (UOp::LessThan(l1, r1), UOp::LessThan(l2, r2)) => {
            pattern_match_impl(l1, l2, mapping) && pattern_match_impl(r1, r2, mapping)
        }

        // 単項演算
        (UOp::Recip(a1), UOp::Recip(a2)) | (UOp::Sqrt(a1), UOp::Sqrt(a2)) => {
            pattern_match_impl(a1, a2, mapping)
        }

        // Select
        (UOp::Select(c1, t1, e1), UOp::Select(c2, t2, e2)) => {
        ...
        // Sequence
        (UOp::Sequence(ops1), UOp::Sequence(ops2)) => {
...
pub fn basic_optimization_rules() -> Vec<RewriteRule> {
    vec![
        ...
        RewriteRule::new(
            "add_zero_right",
            helper::add(helper::wildcard(0), helper::const_val(0.0, DType::F32)),
            |m| m[&0].clone(),
        ),
        // 0 + x = x
        RewriteRule::new(
            "add_zero_left",
            helper::add(helper::const_val(0.0, DType::F32), helper::wildcard(0)),
            |m| m[&0].clone(),
        ),
        // x * 1 = x
        RewriteRule::new(
            "mul_one_right",
            helper::mul(helper::wildcard(0), helper::const_val(1.0, DType::F32)),
            |m| m[&0].clone(),
        ),
        // 1 * x = x
        RewriteRule::new(
            "mul_one_left",
            helper::mul(helper::const_val(1.0, DType::F32), helper::wildcard(0)),
            |m| m[&0].clone(),
        ),
        // x * 0 = 0
        RewriteRule::new(
            "mul_zero_right",
            helper::mul(helper::wildcard(0), helper::const_val(0.0, DType::F32)),
            |_| helper::const_val(0.0, DType::F32),
        ),
        // 0 * x = 0
        RewriteRule::new(
            "mul_zero_left",
            helper::mul(helper::const_val(0.0, DType::F32), helper::wildcard(0)),
            |_| helper::const_val(0.0, DType::F32),
        ),
    ]
}
...
mod tests {
    ...
    fn test_pattern_match_add() {
        let pattern = helper::add(helper::wildcard(0), helper::wildcard(1));
