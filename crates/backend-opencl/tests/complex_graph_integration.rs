//! OpenCL integration tests for complex graphs
//!
//! This test verifies that complex graphs with multiple kernels
//! (including Contiguous nodes) work correctly.

use harp_backend_opencl::{OpenCLCompiler, OpenCLDevice, OpenCLRenderer};
...
/// Helper to get OpenCL device, returns None if unavailable
fn get_device() -> Option<OpenCLDevice> {
    match OpenCLDevice::new() {
        ...
        Err(e) => {
            eprintln!("Skipping test: No OpenCL device available: {}", e);
            None
        }
    }
}
...
///
/// NOTE: Currently ignored due to a pre-existing bug in cast rendering
/// where float bits are reinterpreted as integers instead of proper conversion.
#[test]
#[ignore = "Cast F32->I32 rendering bug - float bits reinterpreted as int"]
