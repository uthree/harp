//! Tensor型の結合テスト
//!
//! autograd の Differentiable が直接 zeros/ones/rand/randn を提供

use harp_nn::tensor::{Tensor0, Tensor1, Tensor2, Tensor3, Tensor4, TensorD};

mod initialization_f32 {
    use super::*;

    #[test]
    fn test_tensor2_zeros() {
...
mod rand_initialization {
    ...
    #[test]
    fn test_tensor2_randn() {
        let tensor = <Tensor2<f32>>::randn(&[3, 4]);
        let lazy_array = tensor.value();
        assert_eq!(lazy_array.shape(), &[3, 4]);
    }

    #[test]
    fn test_tensor1_randn() {
        let tensor = <Tensor1<f32>>::randn(&[10]);
        let lazy_array = tensor.value();
        assert_eq!(lazy_array.shape(), &[10]);
    }

    #[test]
    fn test_tensor3_randn() {
        let tensor = <Tensor3<f32>>::randn(&[2, 3, 4]);
        let lazy_array = tensor.value();
        assert_eq!(lazy_array.shape(), &[2, 3, 4]);
    }

    #[test]
    fn test_tensord_randn() {
        let tensor = <TensorD<f32>>::randn(&[2, 3, 4, 5]);
        let lazy_array = tensor.value();
        assert_eq!(lazy_array.shape(), &[2, 3, 4, 5]);
    }
}
