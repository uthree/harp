...
impl GraphNode {
    ...
    /// ```
    /// use harp_core::prelude::*;
    /// use harp_core::ast::{AstNode, DType as AstDType, Scope, helper::*};
    /// use harp_core::graph::custom_placeholders as ph;
    ///
    /// let mut graph = Graph::new();
    /// let x = graph.input("x", DType::F32, vec![10]);
    ///
    ...
    /// use harp_core::prelude::*;
    /// use harp_core::ast::helper::wildcard;
    /// use harp_core::graph::CumulativeOp;
    ///
    /// let mut graph = Graph::new();
    /// let x = graph.input("x", DType::F32, vec![10, 20]);
    ///
    ...
    /// # 例
    /// ```
    /// use harp_core::graph::{GraphNode, Expr};
    ///
    /// // 静的な形状: 2x3の正規乱数テンソル
