...
/// GELU coefficient for fast approximation
const GELU_COEFF: f64 = 1.702;

impl<T: FloatDType, D: Dimension> Tensor<T, D> {
    /// ReLU activation: max(0, x) (hlop)
    ///
    /// Implemented as: Max(x, 0)
    pub fn relu(&self) -> Tensor<T, D> {
        self.maximum_scalar(T::ZERO)
    }

    /// Leaky ReLU activation (hlop)
    ///
    /// f(x) = x if x > 0, else alpha * x
    pub fn leaky_relu(&self, alpha: T) -> Tensor<T, D> {
        // max(x, alpha * x)
    ...
    pub fn sigmoid(&self) -> Tensor<T, D> {
        let neg_x = -self;
        let exp_neg_x = neg_x.exp();
        let one_plus = exp_neg_x + T::ONE;
        one_plus.recip()
    }
    ...
    pub fn tanh(&self) -> Tensor<T, D> {
        // tanh(x) = (e^(2x) - 1) / (e^(2x) + 1)
        let two_x = self * T::TWO;
        let exp_2x = two_x.exp();
        let numerator = &exp_2x - T::ONE;
        let denominator = &exp_2x + T::ONE;
    ...
    pub fn gelu(&self) -> Tensor<T, D> {
        let coeff = T::from_f64(GELU_COEFF);
        let scaled = self * coeff;
    ...
    ///
    /// f(x) = ln(1 + exp(x))
    pub fn softplus(&self) -> Tensor<T, D> {
        let exp_x = self.exp();
        let one_plus = exp_x + T::ONE;
        one_plus.ln()
    }
    ...
    pub fn elu(&self, alpha: T) -> Tensor<T, D> {
        // For x > 0: x
        // For x <= 0: alpha * (exp(x) - 1)
        let exp_x = self.exp();
        let exp_minus_1 = exp_x - T::ONE;
    ...
    pub fn hardtanh(&self, min_val: T, max_val: T) -> Tensor<T, D> {
        self.maximum_scalar(min_val).min_scalar(max_val)
    }

    /// Element-wise minimum with scalar (hlop)
    pub fn min_scalar(&self, value: T) -> Tensor<T, D> {
        // min(a, b) = -max(-a, -b)
        let neg_self = -self;
        let neg_value = T::ZERO - value;
        let neg_max = neg_self.maximum_scalar(neg_value);
        -neg_max
    }

    /// Clamp values to [min, max] (hlop)
    pub fn clamp(&self, min: T, max: T) -> Tensor<T, D> {
