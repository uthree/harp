    ...
    P: SearchProgress,
{
    ...
    ///
    /// ```ignore
    /// use harp_core::opt::{RuntimeSelector, BeamSearchOptimizer};
    ///
    ...
    ///
    /// ```ignore
    /// use harp_core::opt::progress::{IndicatifProgress, SearchProgress};
    ///
    /// let optimizer = BeamSearchOptimizer::new(suggester)
    ...
    P: SearchProgress,
{
    ...
    pub fn optimize_with_history(&mut self, ast: AstNode) -> (AstNode, OptimizationHistory) {
        ...
        for step in 0..self.max_steps {
            ...
            let mut candidates_with_info: Vec<(AstNode, String, String, super::OptimizationPath)> =
                Vec::new();
