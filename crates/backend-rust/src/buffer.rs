//! CPU buffer implementation for Rust backend
//!
...
/// Error type for RustBuffer operations
#[derive(Debug, Clone)]
pub enum RustBufferError {
    /// Size mismatch error
...
impl fmt::Display for RustBufferError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            RustBufferError::SizeMismatch { expected, actual } => {
                write!(
                    f,
            ...
            RustBufferError::AlignmentError {
                buffer_size,
                type_size,
            } => {
                write!(
...
impl Error for RustBufferError {}

/// CPU buffer for Rust backend
///
...
impl TypedBuffer for RustBuffer {
    type Dev = RustDevice;
    type Error = RustBufferError;
...
impl Buffer for RustBuffer {
    ...
    fn write_from_host(&mut self, data: &[u8]) -> Result<(), Box<dyn Error + Send + Sync>> {
        if data.len() != self.data.len() {
            return Err(Box::new(RustBufferError::SizeMismatch {
...
mod tests {
    ...
    fn test_buffer_allocate() {
        ...
        let buffer = RustBuffer::allocate(&device, vec![2, 3], DType::F32).unwrap();
    ...
    fn test_buffer_write_read() {
        let device = RustDevice::new();
        let mut buffer = RustBuffer::allocate(&device, vec![4], DType::F32).unwrap();
    ...
    fn test_buffer_clone() {
        let device = RustDevice::new();
        let mut buffer = RustBuffer::allocate(&device, vec![2], DType::F32).unwrap();
