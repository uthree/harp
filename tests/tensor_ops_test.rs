...
mod tests {
    ...
    fn test_sliding_window_basic() {
        ...
        // Apply sliding window with window_size=3, stride=1
        // Expected output shape: [2, 3, 8, 3]
        // where 8 = (10 - 3) / 1 + 1
        let windowed: TensorDyn<f32> = a.sliding_window(2, 3, 1);

        assert_eq!(windowed.shape(), &[2, 3, 8, 3]);
        assert_eq!(windowed.ndim(), 4);
    }
    ...
    fn test_sliding_window_with_stride() {
        ...
        // Apply sliding window with window_size=3, stride=2
        // Expected output shape: [1, 1, 4, 3]
        // where 4 = (10 - 3) / 2 + 1
        let windowed: TensorDyn<f32> = a.sliding_window(2, 3, 2);

        assert_eq!(windowed.shape(), &[1, 1, 4, 3]);
        assert_eq!(windowed.ndim(), 4);
    }
    ...
    fn test_sliding_window_invalid_dim() {
        ...
        let _ = a.sliding_window(1, 2, 1); // Should panic, 1D tensor only has axis 0
    }

    #[test]
    fn test_sliding_window_with_grad() {
        use harp::tensor::{Tensor3, TensorDyn};
        ...
        let windowed: TensorDyn<f32> = a.sliding_window(2, 5, 1);

        // Check shape
        // Expected: [1, 1, 26, 5] where 26 = (30 - 5) / 1 + 1
        assert_eq!(windowed.shape(), &[1, 1, 26, 5]);

        // Check that gradient tracking is enabled
        assert!(windowed.is_requires_grad());
    }
}
