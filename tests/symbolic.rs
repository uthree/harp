...
fn test_expr_replace() {
    ...
    let replaced_expr = expr
        .clone()
        .replace(&Expr::Var("x".to_string()), &Expr::from(10));
    ...
    let replaced_index = index_expr
        .clone()
        .replace(&Expr::Index, &Expr::Var("idx0".to_string()));
    assert_eq!(replaced_index, Expr::Var("idx0".to_string()));

    // No replacement if variable not found
    let _z = Expr::Var("z".to_string());
    let replaced_expr_3 = expr
        .clone()
        .replace(&Expr::Var("z".to_string()), &Expr::from(100));
