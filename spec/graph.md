...
> **Note**: これはharpの低レベルAPIです。通常のユースケースでは[Tensor API](tensor.md)を推奨します。
> GraphNodeは`harp::prelude`には含まれていないため、使用する場合は`use harp::graph::GraphNode;`で明示的にインポートしてください。
...
この低レベルAPIは、カスタム最適化やデバッグ目的での直接的なグラフ操作が必要な場合に使用します。
...
SIMD化はAST最適化フェーズで`VectorizationSuggester`により行われます。
`LoweringSuggester`はスカラー版のコードのみを生成します。

詳細は`spec/opt-ast.md`のVectorizationSuggesterセクションを参照してください。
