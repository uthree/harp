...
`OptimizationHistory`は最適化の各ステップを記録し、可視化やデバッグに使用できる。
...
- `ast`: その時点でのAST
- `cost`: コスト推定値
- `num_candidates`: そのステップでSuggesterが提案した候補数（Optional）
- `logs`: デバッグログ

### メソッド
- `cost_transition()`: コストの遷移を`Vec<(usize, f32)>`で取得
- `candidate_transition()`: 候補数の遷移を`Vec<(usize, usize)>`で取得

可視化では、コストと候補数を同一グラフ上にスケーリングして表示する。候補数の推移を見ることで、最適化がどの段階で収束に向かっているかを把握できる。
