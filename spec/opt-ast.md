...
**重要**: 現在のループ融合実装はデータ依存関係分析を行っていない。

以下のケースで不正な融合が発生する可能性:
- **Write-After-Read依存**: 後のループが前のループで読んだデータを書き換える
- **Read-After-Write依存**: 後のループが前のループで書いたデータを読む
- **Write-After-Write依存**: 両ループが同じメモリ位置に書き込む

```
// 危険なケース
for i in 0..100 { a[i] = b[i] }      // ループ1
for i in 0..100 { b[i] = a[i] + 1 }  // ループ2

// 融合すると依存関係が壊れる:
for i in 0..100 {
    a[i] = b[i]      // b[i]を読む
    b[i] = a[i] + 1  // b[i]を書く（次のイテレーションで問題）
}
```

将来的にはメモリアクセスパターンの解析と依存グラフの構築が必要。
